...
const PlayerContent: FC<PlayerContentProps> = ({
  ...
  const renderLeft = () => {
    ...
    return (
      <div className="flex items-center flex-grow mr-2 overflow-hidden lg:flex-shrink-0 lg:basis-52">
        <Link
          href={href}
          className="relative flex items-center pl-12 space-x-2 overflow-hidden h-14 sm:h-16 sm:space-x-3"
        >
          <NcImage
            alt={title}
            fill
  ...
  const renderDurationTime = () => {
    return (
      <div className="absolute inset-x-0 w-full bottom-full">
        <input
          className="absolute inset-0 z-10 w-full h-full opacity-0 cursor-pointer slider"
          type="range"
          min={0}
          max={0.999999}
          step="any"
        ...
        >
          <span className="absolute w-3 h-3 transform -translate-y-1/2 rounded-full -right-3 top-1/2 bg-primary-500"></span>
        </div>
      </div>
    );
  };
  ...
  const renderButtonControl = () => {
    return (
      <div
        className="flex items-center justify-center flex-shrink-0 transition-colors rounded-full cursor-pointer w-14 h-14 sm:w-16 sm:h-16 text-neutral-700 dark:text-neutral-200 bg-neutral-100/70 hover:bg-neutral-200/70 dark:bg-neutral-700/40 dark:hover:bg-neutral-700/80"
  ...
  const renderButtonControlMobile = () => {
    return (
      <div
        className="flex items-center justify-center flex-shrink-0 w-10 h-10 rounded-full cursor-pointer lg:hidden text-neutral-700 dark:text-neutral-200 hover:bg-neutral-100 dark:hover:bg-black/10 "
        onClick={handleClickToggle}
      >
        {/* PLAYING */}
        {playing ? (
  ...
  const renderForwards15S = () => {
    return (
      <button
        className="flex items-center justify-center w-12 h-12 transition-colors rounded-full hover:bg-neutral-100 dark:hover:bg-neutral-700/80"
  ...
  const renderContentCenter = () => {
    return (
      <div className="flex items-center flex-grow max-w-xs text-neutral-500 dark:text-neutral-300 justify-evenly xl:max-w-md">
  ...
  const renderVolumn = () => {
    return (
      <div className="items-center justify-end hidden lg:flex text-neutral-500 dark:text-neutral-300">
        <button
          onClick={() => {
            if (!volume) {
        ...
        <div className="ml-3.5 relative w-24 flex-shrink-0">
          <input
            className="absolute inset-0 z-10 w-full h-1 opacity-0 cursor-pointer slider "
            type="range"
            min={0}
            max={0.999999}
            step="any"
  ...
  const renderClose = () => {
    return (
      <button
        className="flex items-center justify-center flex-shrink-0 w-10 h-10 rounded-full focus:outline-none focus:shadow-outline hover:bg-neutral-100 dark:hover:bg-neutral-700/80 md:w-12 md:h-12"
  ...
  const renderTiming = () => {
    return (
      <div className="items-center justify-center flex-shrink-0 hidden ml-5 mr-3 text-xs tracking-widest lg:flex">
  ...
  return (
    <>
      <Transition
        className="relative z-0 flex flex-col w-full px-2 bg-white dark:bg-neutral-800 sm:px-3 - nc-google-shadow"
      ...
      >
        ...
        >
          <div className="flex items-center justify-center w-6 h-6 bg-white rounded-full dark:bg-neutral-800 - nc-google-shadow">
            <ChevronUpIcon
        ...
        <div className="flex justify-between w-full h-16 sm:h-20">
          {/* LEFT */}
          {renderLeft()}

          {/* CENTER */}
          <div className="items-center justify-center flex-grow hidden px-5 lg:flex">
            {isError ? (
          ...
          {/* RENDER RIGHT */}
          <div className="flex items-center justify-end flex-shrink-0 ml-2 lg:basis-52 lg:flex-grow">
            {renderVolumn()}
            {renderTiming()}
        ...
        <Transition
          className="z-0 flex justify-center h-16 transition-all border-t lg:hidden border-neutral-300 dark:border-neutral-700 "
          enter="duration-150"
          enterFrom="-mb-16"
          enterTo="mb-0"
          leave="duration-150"
        ...
        >
          <div className="flex items-center flex-grow max-w-xs justify-evenly text-neutral-500 dark:text-neutral-300 sm:max-w-sm md:max-w-md ">
