...
const SectionVideos: FC<SectionVideosProps> = ({
  ...
  const renderSubVideo = (video: VideoType, index: number) => {
    ...
    return (
      <div
        className="relative z-0 overflow-hidden cursor-pointer group aspect-h-16 aspect-w-16 rounded-2xl sm:aspect-h-12 sm:rounded-3xl lg:aspect-h-9"
        onClick={() => {
  ...
  return (
    <div className={`nc-SectionVideos ${className}`}>
      <Heading
        desc="Découvrez nos vidéos les plus chaudes. Voir plus et partager plus de nouveautés
          perspectives sur à peu près n’importe quel sujet. Tout le monde est le bienvenu."
      >
        🎬 Les vidéos
      </Heading>
