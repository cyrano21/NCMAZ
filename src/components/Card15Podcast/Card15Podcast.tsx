  ...
  const renderDefaultBtnListen = (state?: "playing") => {
    return (
      <div className="inline-flex items-center mt-3 pr-4 py-0.5 hover:pl-0.5 cursor-pointer rounded-full transition-all hover:bg-primary-50 dark:hover:bg-neutral-900">
        <span className="flex items-center justify-center w-8 h-8 rounded-full bg-primary-50 dark:bg-neutral-800 text-primary-6000 dark:text-primary-200">
          {state === "playing" ? (
        ...
        <span className="ml-3 text-xs font-medium sm:text-sm">
          {state === "playing" ? "en cours" : "Écoute maintenant"}
        </span>
      </div>
    );
  };
  ...
  return (
    ...
    >
      <div className="flex-shrink-0 w-1/4">
        <Link
          href={href}
          className="relative block h-0 overflow-hidden rounded-full shadow-lg aspect-w-1 aspect-h-1"
        >
          <Image
